...
RegisterNetEvent('Galaxy_Marihuana_Plantada:UseSeed')
RegisterNetEvent('Galaxy_Marihuana_Plantada:UseItem')
RegisterNetEvent('Galaxy_Marihuana_Plantada:UseBag')
RegisterNetEvent('Galaxy_Marihuana_Plantada:UseRollingPapers')
...
function GMP:Start()
  ...
  ESX.TriggerServerCallback('Galaxy_Marihuana_Plantada:GetLoginData', function(plants)  
...
function GMP:GetLoginData()
end

function GMP:PerSecThread()
  while true do
    Wait(1000)
  ...
  if self.CanHarvest or self.PolText then 
    if Utils:GetKeyPressed("E") and (GetGameTimer() - self.timer) > 200 and not self.CurInteracting then
      Citizen.CreateThread(function()
        ...
        Citizen.Wait(1000)
        exports['progressBars']:startUI(20000, "Cortando")
...
function GMP:TextHandler()
  ...
  if closest.Growth >= 99.99 then 
    ...
    if closest.Owner == plyId  then
      strA = strA .. "[ Presiona [ E ] Para Cortar ] " 
  ...
  if (plyData.job and plyData.job.label and plyData.job.label == self.PoliceJobLabel) then
    self.PolText = { closest = closest, strA = "[ Presiona [~r~ E ~s~] para destruir ]" }
  end
  self.CurKey = closestKey
end

function GMP:GetValColour(v)
...
function GMP:GrowthHandlerSlow()
  if not self.Plants then return; end
  if not #self.Plants then return; end
  local plyData = ESX.GetPlayerData()
  local plyId = plyData.identifier
...
function GMP:GrowPlantSlow(plant,key)
  if not self.Plants then return; end
...
function GMP:GrowthHandlerFast()
  if not self.Plants then return; end
  if not #self.Plants then return; end
  local plyData = ESX.GetPlayerData()
  local plyId = plyData.identifier
...
function GMP:GrowPlantFast(plant,key)
  if not self.Plants then return; end
  if not self.Plants[key] then return; end
  if self.Plants[key] ~= plant then return; end
...
function GMP:EnterInstance(instance)
  self.Instance = instance.data.owner
  print("Enter Instance : GMP")
  if not self.Plants then return; end
  if not #self.Plants then return; end
  for k = 1,#self.Plants,1 do
    local v = self.Plants[k]
    if v and v.Instance then
      if v.Instance == self.Instance then
        GMP:SpawnInstance(v,k)
      end
    end
  end
end

function GMP:LeaveInstance(instance)
  print("Leave Instance : GMP")
  if not self.Plants then return; end
  if not #self.Plants then return; end
  self.Instance = false
  for k = 1,#self.Plants,1 do
...
function GMP:SpawnInstance(plant,k)
...
function GMP:SpawnWorld(plant,k)
...
function GMP:SyncHandler(plant,delete)
...
function GMP:Awake(...)
    ...
    ESX.TriggerServerCallback('Galaxy_Marihuana_Plantada:GetStartData', function(retVal) self.dS = true; self.cS = retVal; end)
    while not self.dS do Citizen.Wait(0); end
    self:Start()
end

function GMP:GetPlantZ(plant)
  if plant.Stage <= 3 then return -1.0
  else return -3.5
  end
end

function GMP:UseItem(item)
  if not self.Plants then return; end
  if not #self.Plants then return; end
...
function GMP:Sync(plant,delete)
  TriggerServerEvent('Galaxy_Marihuana_Plantada:SyncPlant',plant,delete)
end

function GMP:UseBag(canUse, msg)
  Citizen.CreateThread(function(...)
    local plyPed = GetPlayerPed(-1)
...
function GMP:DrawText3D(x,y,z, text)
...
AddEventHandler('instance:onEnter', function(instance) while not GMP.iR do Citizen.Wait(0); end GMP:EnterInstance(instance); end)
AddEventHandler('instance:onLeave', function(instance) if GMP.iR then GMP:LeaveInstance(instance); else GMP.Instance = false; end; end)
AddEventHandler('Galaxy_Marihuana_Plantada:UseSeed', function(seed) if GMP.iR then GMP:UseSeed(seed); end; end)
AddEventHandler('Galaxy_Marihuana_Plantada:UseItem', function(item) if GMP.iR then GMP:UseItem(item); end; end)
AddEventHandler('Galaxy_Marihuana_Plantada:SyncPlant', function(plant,del) if GMP.iR then GMP:SyncHandler(plant,del); end; end)
AddEventHandler('Galaxy_Marihuana_Plantada:UseBag', function(canUse,msg) GMP:UseBag(canUse,msg); end)
